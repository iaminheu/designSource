...
type
  TFrm_Pack = class(TFrm_GridBass)
    ...
    FFileItemID_2: TcxGridDBBandedColumn;
    ...
    TabSheet6: TTabSheet;
    cxGrid_PackList: TcxGrid;
    cxGV_PackList: TcxGridDBTableView;
    ...
    TabSheet7: TTabSheet;
    cxGrid_PackItem: TcxGrid;
    cxGridDBTableView2: TcxGridDBTableView;
    cxGV_PackItem: TcxGridDBBandedTableView;
    ...
    cxGL_PackItem: TcxGridLevel;
    MData_PackItem: TdxMemData;
    ...
    DS_Mdata_PackItem: TDataSource;
    DS_Mdata_PackList: TDataSource;
    MData_PackList: TdxMemData;
    ...
    procedure MyTreeView_PackItemChange(Sender: TObject; Node: TTreeNode);
    procedure MyTreeView_PackItemExpanding(Sender: TObject;
      Node: TTreeNode; var AllowExpansion: Boolean);
    procedure cxGV_PackItemFocusedRecordChanged(
...
function TFrm_Pack.DelRdPackItem: bool;
var
 qry: TADOQuery;
begin
...
function TFrm_Pack.EditRd:bool;
var
 qry:TADOQuery;
begin
  qry:=TADOQuery.Create(Self);
...
procedure TFrm_Pack.FormResize(Sender: TObject);
begin
  inherited;
  Self.Caption:=UserFDepartmentName+'包装入库清单';
end;

procedure TFrm_Pack.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action:=cafree;
  Frm_Pack:=nil;
...
procedure TFrm_Pack.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.MyTreeView_SListExpanding(Sender: TObject;
  Node: TTreeNode; var AllowExpansion: Boolean);
var
  P :PTree;
  qry:TADOquery;
...
procedure TFrm_Pack.MyTreeView_SListChange(Sender: TObject; Node: TTreeNode);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
    ...
    Case PTree(Node.Data).Isleaf of
      ...
      begin
        ...
        begin
          ...
          TB_Prin3.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'打印');
        end;
      end;
      3:
      begin
...
procedure TFrm_Pack.frReportBeginBand(Band: TfrBand);
begin
  inherited;
...
procedure TFrm_Pack.TB_Ref3Click(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_Pack.FormShow(Sender: TObject);
begin
  inherited;
  TB_Ref.Click;
  TB_Ref2.Click;
...
procedure TFrm_Pack.TB_Out3Click(Sender: TObject);
Var i:Integer;
begin
  inherited;
...
procedure TFrm_Pack.TB_Exit3Click(Sender: TObject);
begin
  inherited;
  Self.Close;
end;

procedure TFrm_Pack.TB_Prin3Click(Sender: TObject);
...
procedure TFrm_Pack.cxGV_SItemFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
var qry: TADOQuery;
...
procedure TFrm_Pack.cxGV_SLIstCustomDrawCell(
...
procedure TFrm_Pack.FFactureName_MPropertiesButtonClick(
...
procedure TFrm_Pack.TB_RefClick(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_Pack.MyTreeViewExpanding(Sender: TObject;
  Node: TTreeNode; var AllowExpansion: Boolean);
var
  P :PTree;
  qry:TADOquery;
...
procedure TFrm_Pack.MyTreeViewChange(Sender: TObject;
  Node: TTreeNode);
var
  qry:TADOquery;
begin
  ...
  begin
    ...
    Case PTree(Node.Data).Isleaf of
      ...
      begin
        Act_Status_M.Execute;  //权限设置多行
        TB_IMP_PackItem.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'修改');
      end;
      2:
      begin
        Act_Status_M.Execute;  //权限设置多行
...
procedure TFrm_Pack.cxGV_MFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
var qry: TADOQuery;
...
procedure TFrm_Pack.AddPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
...
procedure TFrm_Pack.TB_Edit_MClick(Sender: TObject);
begin
  inherited;
  P_state :=1;     //修改
...
procedure TFrm_Pack.OKBtn_MClick(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_Pack.CancelBtn_MClick(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.TB_UpdateClick(Sender: TObject);
var
  proc:TADOStoredProc;
  P_Retinf1:String;
...
procedure TFrm_Pack.TB_StockPackClick(Sender: TObject);
var
  proc:TADOStoredProc;
  P_Retinf1:String;
  qry: TADOQuery;
...
procedure TFrm_Pack.TB_Ref2Click(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
  qry.ConnectionString:=mdlData.ReadConnStr;
  If not TreeVeiwListWhere(qry,MyTreeView_Pack,'VT_Pack',' and FDepartmentID='+IntToStr(UserFDepartmentID)) then
      Application.MessageBox(PChar('树型结构显示错误，请联系系统开发人员'), '错误', MB_OK +MB_ICONSTOP);
end;

procedure TFrm_Pack.MyTreeView_PackExpanding(Sender: TObject;
  Node: TTreeNode; var AllowExpansion: Boolean);
var
  P :PTree;
  qry:TADOquery;
...
begin
  ...
  If (Node = nil) or
   ...
   qry.SQL.Text:='Select * from VT_Pack where FParentNumber= '''
               +PTree(Node.Data).ID+''' and FDepartmentID=:FDepartmentID  order by FBranchItemNumber,FNumber';
...
procedure TFrm_Pack.MyTreeView_PackChange(Sender: TObject;
  Node: TTreeNode);
var
  qry:TADOquery;
begin
...
procedure TFrm_Pack.TB_Edit_M2Click(Sender: TObject);
begin
  P_state:=1;     //修改
  P_Move:=0;
...
procedure TFrm_Pack.MData_PItemBeforePost(DataSet: TDataSet);
Var i:Integer;
begin
  inherited;
  i:=DataSet['FPackNo'];
...
procedure TFrm_Pack.CancelBtn2Click(Sender: TObject);
begin
  inherited;
  P_Move:=1;
...
procedure TFrm_Pack.OKBtn2Click(Sender: TObject);
var qry: TADOQuery;
begin
  qry:=TADOQuery.Create(self);
  qry.ConnectionString:=mdlData.ReadConnStr;
  ...
  begin
    try
      qry.SQL.Text:='Update T_PackItem set FPackNo=:FPackNo,FPackClass=:FPackClass,FPackNumber=:FPackNumber,FPackName=:FPackName,FPackMode=:FPackMode,FLength=:FLength,FWidth=:FWidth,FHigh=:FHigh,'
                   +'FPlace=:FPlace,FSteel=:FSteel,FWood=:FWood,FPlastic=:FPlastic,FAsphalt=:FAsphalt,FColorbar=:FColorbar,FFileItemID=:FFileItemID,FPackUserNum=:FPackUserNum,FPackUserNumDate=:FPackUserNumDate '
      ...
      begin
        ...
        FindParam('FFileItemID').value:=mdata_PItem['FFileItemID'];
...
procedure TFrm_Pack.TB_IMP_PackItemClick(Sender: TObject);
 var
  proc:TADOStoredProc;
  qry: TADOQuery;
begin
  If ShowMsg('是否将【'+P_th+'】下推行到箱件清单？','提示') then
  begin
...
procedure TFrm_Pack.cxGV_PItemFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
var qry: TADOQuery;
begin
  ...
  begin
    ...
    begin
      If MyItemList(qry,Mdata_PList,'V_PackItemList','cast(FDepartmentID as nvarchar(20))+'';''+FItemNo+'';''+FPackNum',mdata_PItem['FFullNumber'],'FFullNumber') then
    end;
  end;

end;

procedure TFrm_Pack.FPackClassHeaderClick(Sender: TObject);
Var
  qry: TADOQuery;
  I_index:Integer;
  P_Sel:String;
...
procedure TFrm_Pack.TB_DelClick(Sender: TObject);
var
  qry:TADOQuery;
begin
  qry:=TAdoQuery.Create(self);
...
procedure TFrm_Pack.TB_ReNoRefClick(Sender: TObject);
var
  IsEdit:bool;
  qry: TADOQuery;
  I_index:integer;
...
procedure TFrm_Pack.TB_PackItemClick(Sender: TObject);
begin
  inherited;
  P_Move:=0;
...
procedure TFrm_Pack.TB_PackListClick(Sender: TObject);
var   qry: TADOQuery;
begin
  P_Move:=0;
  qry:=TADOQuery.Create(self);
  ...
  try
    qry.SQL.Text:='select * from V_PackItemList where FPackNo<>0 and cast(FBillDepartmentID as nvarchar(20))+'';''+FItemNo+'';''+FPackNum like :FFullNumber order by FItemNumber,FPackNum,FListNum ';
  ...
  try
    qry.SQL.Text:='select * from V_PackItemList where cast(FBillDepartmentID as nvarchar(20))+'';''+FItemNo+'';''+FPackNum like :FFullNumber and FPackNum>=:FPackNum1 and FPackNum<=:FPackNum2 order by FItemNumber,FPackNum,FListNum ';
    qry.Parameters.FindParam('FFullNumber').value:=ListFNumber_Pack+'%';
...
procedure TFrm_Pack.frReport1BeginBand(Band: TfrBand);
begin
  inherited;
...
procedure TFrm_Pack.FPlaceHeaderClick(Sender: TObject);
Var
  qry: TADOQuery;
  I_index:Integer;
  P_Sel:String;
...
procedure TFrm_Pack.FPackMode_2HeaderClick(Sender: TObject);
Var
  qry: TADOQuery;
  I_index:Integer;
  P_Sel:String;
...
procedure TFrm_Pack.TB_PackItemFirstClick(Sender: TObject);
begin
  inherited;
  P_Move:=0;
  If mdata_PItem.RecordCount>0 then
...
procedure TFrm_Pack.E_FPackNo1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var selValue : string;
    qry: TADOQuery;
begin
...
procedure TFrm_Pack.E_FPackNum2PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var selValue : string;
    qry: TADOQuery;
begin
...
procedure TFrm_Pack.cxGV_PListDblClick(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.FPackNumber_2HeaderClick(Sender: TObject);
Var
  qry: TADOQuery;
  I_index:Integer;
  P_Sel:String;
...
procedure TFrm_Pack.cxGV_PItemDblClick(Sender: TObject);
Var
...
procedure TFrm_Pack.TB_PackDraftClick(Sender: TObject);
var   qry: TADOQuery;
begin
  P_Move:=0;
  qry:=TADOQuery.Create(self);
  qry.ConnectionString:=mdlData.ReadConnStr;
  try
    qry.SQL.Text:='select * from V_PackDraft where FFileItemID=:FFileItemID and FPackNum>=:FPackNum1 and FPackNum<=:FPackNum2  order by FFileItemID,FPackNum,FListNum ';
    qry.Parameters.FindParam('FFileItemID').value:=mdata_PItem['FFileItemID'];
    qry.Parameters.FindParam('FPackNum1').value:=Trim(E_FPackNum1.Text);
    qry.Parameters.FindParam('FPackNum2').value:=Trim(E_FPackNum2.Text);
   qry.Open;
...
procedure TFrm_Pack.TB_Out2Click(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.TB_Down2Click(Sender: TObject);
Var i:Integer;
begin
  If P_i then
  begin
...
procedure TFrm_Pack.TB_DownClick(Sender: TObject);
Var i:Integer;
begin
  If P_i then
  begin
...
procedure TFrm_Pack.cxGVDblClick(Sender: TObject);
Var
  P_FPackNo:Integer;
  P_FPackClass:string;
begin
...
procedure TFrm_Pack.APPPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
  Act_App.Execute;
...
procedure TFrm_Pack.NextPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
  Act_Next.Execute;
...
procedure TFrm_Pack.Act_SetOpen_M2Execute(Sender: TObject);
begin
  inherited;
  cxGV_PItem.OptionsData.Editing:=True;
  OKBtn2.Visible:=True;
...
procedure TFrm_Pack.Act_SetClose_M2Execute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.Act_SetOpen_MExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.Act_SetClose_MExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.TB_PackListXLSClick(Sender: TObject);
var
...
begin
  ...
  Begin
    ...
    try
      qry.SQL.Text:='select distinct FBranchItemNumber  from V_PackDraft where FFileItemID=:FFileItemID and FPackNum>=:FPackNum1 and FPackNum<=:FPackNum2 order by FBranchItemNumber ';
      qry.Parameters.FindParam('FFileItemID').value:=mdata_PItem['FFileItemID'];
      qry.Parameters.FindParam('FPackNum1').value:=Trim(E_FPackNum1.Text);
      qry.Parameters.FindParam('FPackNum2').value:=Trim(E_FPackNum2.Text);
      qry.Open;
   ...
   begin
     try
       qry1.SQL.Text:='select * from V_PackDraft where FFileItemID=:FFileItemID and FPackNum>=:FPackNum1 and FPackNum<=:FPackNum2 order by FBranchItemNumber,FPackNum,FListNum ';
       qry1.Parameters.FindParam('FFileItemID').value:=mdata_PItem['FFileItemID'];
...
procedure TFrm_Pack.MDataAfterEdit(DataSet: TDataSet);
begin
  inherited;
  DataSet['IsEdit']:=True;
end;

procedure TFrm_Pack.TB_Edit2Click(Sender: TObject);
begin
  inherited;
  P_state:=1;
...
procedure TFrm_Pack.OKBtn_2Click(Sender: TObject);
var
  IsEdit:bool;
  qry: TADOQuery;
  I_index:integer;
...
procedure TFrm_Pack.Act_IMP_PackItemExecute(Sender: TObject);
var
  proc:TADOStoredProc;
  qry: TADOQuery;
begin
...
procedure TFrm_Pack.CancelBtn_2Click(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.SelPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
...
procedure TFrm_Pack.TB_PrinClick(Sender: TObject);
begin
  If mdata.RecordCount>0 then
  begin
...
procedure TFrm_Pack.TB_EditClick(Sender: TObject);
begin
  inherited;
  P_state:=1;
  Act_SetOpen.Execute;
...
procedure TFrm_Pack.CancelBtnClick(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.OKBtnClick(Sender: TObject);
var
  IsEdit:bool;
  qry: TADOQuery;
  I_index:integer;
...
procedure TFrm_Pack.FormDestroy(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.ActPackMXExecute(Sender: TObject);
Var
  P_FLength,P_FWidth,P_FHigh:Integer;
  P_FSteel,P_FWood,P_FPlastic,P_FAsphalt,P_FColorbar:Real;
...
procedure TFrm_Pack.Act_AppExecute(Sender: TObject);
var
  IsSel:bool;
  I_index:integer;
  P_FPackNo:Integer;
...
procedure TFrm_Pack.Act_NextExecute(Sender: TObject);
var
  IsSel:bool;
  I_index,i:integer;
  P_FPackNo:Integer;
...
procedure TFrm_Pack.Act_AddExecute(Sender: TObject);
...
procedure TFrm_Pack.Act_SelExecute(Sender: TObject);
Var
  i,j: integer;
begin
  inherited;
...
procedure TFrm_Pack.TB_Pack_SGClick(Sender: TObject);
var
  Query_Sp:TADOStoredProc;
  i,k,s,ReNum,ReNumLoop,R_Count,L:Integer;
  ExcelApp,MyWorkBook,mysheet,vLastSheet: Variant;
...
begin
  ...
  Begin
    ...
    try
      qry.SQL.Text:='select distinct FBranchItemNumber,FPackNum  from V_PackItemList where cast(FBillDepartmentID as nvarchar(20))+'';''+FItemNo+'';''+FPackNum like :FFullNumber and FPackNum>=:FPackNum1 and FPackNum<=:FPackNum2 order by FBranchItemNumber,FPackNum ';
      qry.Parameters.FindParam('FFullNumber').value:=ListFNumber_Pack+'%';
      qry.Parameters.FindParam('FPackNum1').value:=Trim(E_FPackNum1.Text);
      qry.Parameters.FindParam('FPackNum2').value:=Trim(E_FPackNum2.Text);
      qry.Open;
   ...
   begin
     try
       qry1.SQL.Text:='select * from V_PackItemList where cast(FBillDepartmentID as nvarchar(20))+'';''+FItemNo+'';''+FPackNum like :FFullNumber and FPackNum=:FPackNum order by FItemNumber,FPackNum,FListNum ';
...
procedure TFrm_Pack.TB_PackDraftXlsClick(Sender: TObject);
var
  Query_Sp:TADOStoredProc;
  i,k,s,ReNum,ReNumLoop,R_Count,L:Integer;
  ExcelApp,MyWorkBook,mysheet,vLastSheet: Variant;
...
procedure TFrm_Pack.TB_SubmitClick(Sender: TObject);
var
 qry:TADOquery;
begin
  qry:=TAdoQuery.Create(self);
...
procedure TFrm_Pack.Act_StatusExecute(Sender: TObject);
begin
  inherited;
  Case mdata_M['FPackStockStatus'] of
    ...
    begin
      ...
      begin
        TB_Edit_M.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'修改');
        TB_Edit.Visible:=False;
        TB_Del.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'删除');
        ToolBar.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'修改');
        TB_Submit.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'提交');
      end
      else
      begin
        TB_Del.Visible:=False;
  ...
  Case mdata_M['FPackStockStatus'] of
    ...
    begin
      ...
      TB_Audit.Caption:='审核';
      TB_Audit.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'审核');
      TB_Del.Visible:=False;
      TB_Edit_M.Visible:=False;
      TB_Edit.Visible:=False;
      ToolBar.Visible:=False;
...
procedure TFrm_Pack.Act_Status_MExecute(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
 ...
 // ToolBar.Visible :=False;
  ...
  //填单者是本人
  try
    qry.SQL.Text:='Select distinct FPackStockStatus,FPackStockWriteID from V_PackStockItem where FPackStockWriteID=:FPackStockWriteID and '
                 +' cast(FBIllDepartmentID as Varchar(20))+'';''+FFullNumber like :FFullNumber ';
...
//  If (qry.RecordCount=1) and (qry.FieldByName('FPackStockStatus').AsInteger=0) and (qry.FieldByName('FPackStockWriteID').AsInteger=UserNumID) then
  If (qry.RecordCount=1) and (qry.FieldByName('FPackStockWriteID').AsInteger=UserNumID) then
  begin
    TB_Edit_M.Visible:=False;
    ToolBar.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'修改');
    TB_Edit.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'修改');
    TB_Del.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'删除');
    //TB_Del.Visible:=False;
    TB_Submit.Visible:=mdlData.CheckFrmRights(TFrm_Pack.ClassName,'提交');
    TB_Submit.Caption:='提交';
   ...
   //提交者是本人
  try
    qry.SQL.Text:='Select distinct FPackStockStatus,FPackStockSubmitID from V_PackStockItem where FPackStockSubmitID=:FPackStockSubmitID '
                 +' and cast(FBIllDepartmentID as Varchar(20))+'';''+FFullNumber like :FFullNumber ';
    qry.Parameters.FindParam('FPackStockSubmitID').value:=UserNumID;
    qry.Parameters.FindParam('FFullNumber').value:=IntToStr(UserFDepartmentID)+';'+ListItemFNumber+'%';
    qry.Open;
  finally
  ...
  //审核者是本人
  try
    qry.SQL.Text:='Select distinct FPackStockStatus,FPackStockAuditID from V_PackStockItem where FPackStockAuditID=:FPackStockAuditID '
                 +' and cast(FBIllDepartmentID as Varchar(20))+'';''+FFullNumber like :FFullNumber ';
...
procedure TFrm_Pack.cxGVCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
  inherited;
...
procedure TFrm_Pack.N6Click(Sender: TObject);
Var
  P_FLength,P_FWidth,P_FHigh:Integer;
  P_FSteel,P_FWood,P_FPlastic,P_FAsphalt,P_FColorbar:Real;
  P_FPackNumber,P_FPackName,P_FPackMode,P_FPlace,a:string;
...
procedure TFrm_Pack.TB_PackItemXLSClick(Sender: TObject);
var
  Query_Sp:TADOStoredProc;
  i,k,s,ReNum,ReNumLoop,R_Count,L:Integer;
  ExcelApp,MyWorkBook,mysheet,vLastSheet: Variant;
...
begin
  ...
  try
    qry.SQL.Text:='select * from V_PackItem where FFileItemID=:FFileItemID and FPackNum>=:FPackNum1 and FPackNum<=:FPackNum2  order by FFileItemID,FPackNum ';
    qry.Parameters.FindParam('FFileItemID').value:=mdata_PItem['FFileItemID'];
    qry.Parameters.FindParam('FPackNum1').value:=Trim(E_FPackNum1.Text);
    qry.Parameters.FindParam('FPackNum2').value:=Trim(E_FPackNum2.Text);
   qry.Open;
...
procedure TFrm_Pack.Act_SetClose_Tree2Execute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.Act_SetOpen_Tree2Execute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.E_nPropertiesChange(Sender: TObject);
begin
  inherited;
  E_L.EditValue:=mdata['FSumQry']/E_n.EditValue;
end;

procedure TFrm_Pack.Act_SetOpen_2Execute(Sender: TObject);
begin
  inherited;
  MyTreeView_Pack.Enabled:=False;
  ToolBar_M2.Enabled:=False;
...
procedure TFrm_Pack.Act_SetClose_2Execute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.Act_SetOpenExecute(Sender: TObject);
begin
  inherited;
  cxGV.OptionsData.Editing:=False;
  MyTreeView.Enabled:=False;
...
procedure TFrm_Pack.Act_SetCloseExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_Pack.TB_AutoClick(Sender: TObject);
var
  proc:TADOStoredProc;
  P_Retinf:String;
  qry: TADOQuery;
...
procedure TFrm_Pack.FPackNumber_2PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var  qry: TADOQuery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_Pack.FPlacePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var selValue : string;
    qry: TADOQuery;
begin
...
procedure TFrm_Pack.ToolButton1Click(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
  qry.ConnectionString:=mdlData.ReadConnStr;
  If not TreeVeiwListWhere(qry,MyTreeView_PackItem,'VT_Pack',' and FDepartmentID='+IntToStr(UserFDepartmentID)) then
      Application.MessageBox(PChar('树型结构显示错误，请联系系统开发人员'), '错误', MB_OK +MB_ICONSTOP);

end;

procedure TFrm_Pack.MyTreeView_PackItemChange(Sender: TObject;
  Node: TTreeNode);
var
  qry:TADOquery;
begin
  ...
  begin
    ...
    If PTree(Node.Data).Isleaf>0 then
    begin
      If MyItemList(qry,Mdata_PackItem,'V_PackItem','FFullNumber',ListFNumber_Pack,'FFullNumber') then
      cxGvColumn(cxGv_PackList,Mdata_PackList);
      P_Move:=1;
    end
    else
    begin
      Mdata_PackItem.Close;
      Mdata_PackList.Close;
    end;
...
procedure TFrm_Pack.MyTreeView_PackItemExpanding(Sender: TObject;
  Node: TTreeNode; var AllowExpansion: Boolean);
var
  P :PTree;
  qry:TADOquery;
begin
  ...
  begin
    ...
    P.Isleaf := qry.FieldByName('Isleaf').Value;
    P.Flag := cNoLoadFlag;                            // 新节点，未加载标志
    with MyTreeView_PackItem.Items.AddChildObject(Node,P.Caption,P) do
    begin
       HasChildren :=not qry.fieldbyname('Isleaf').value;
    end;
    qry.Next;
...
procedure TFrm_Pack.cxGV_PackItemFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
var qry: TADOQuery;
begin
  ...
  If P_Move=1 then
  begin
    If (mdata_PackItem['FFullNumber']<>'') and (mdata_PackItem['FFullNumber']<>Null) and (mdata_PackItem.RecordCount>0) then
    begin
      If MyItemList(qry,Mdata_PackList,'V_PackList','cast(FDepartmentID as nvarchar(20))+'';''+FItemNo+'';''+FPackNum',mdata_PackItem['FFullNumber'],'FFullNumber') then
    end;
  end;
