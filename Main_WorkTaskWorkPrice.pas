...
uses
  ...
  cxSpinEdit, ActnList, Menus;

type
  TFrm_WorkTaskWorkPrice = class(TFrm_GridBass)
    MData: TdxMemData;
    ...
    procedure Act_TS_WorkTaskWorkPriceExecute(Sender: TObject);
...
var
  Frm_WorkTaskWorkPrice: TFrm_WorkTaskWorkPrice;
...
function TFrm_WorkTaskWorkPrice.DelRd: bool;
var
 qry: TADOQuery;
begin
  ...
  try
    qry.SQL.Text:='Delete from T_WorkTaskWorkPrice where FWorkPriceID=:FWorkPriceID';
...
function TFrm_WorkTaskWorkPrice.AddRd: bool;
var
  qry: TADOQuery;
...
begin
  ...
  try
    qry.SQL.Text:='Insert into T_WorkTaskWorkPrice(FWorkTaskPartsID, FWorkBasePrice, FWorkResizePrice,FWorkCarryJobID) '
                 +'values(:FWorkTaskPartsID, :FWorkBasePrice, :FWorkResizePrice, :FWorkCarryJobID)';
    mData.First;
...
function TFrm_WorkTaskWorkPrice.EditRd:bool;
var
  qry: TADOQuery;
begin
  qry:=TADOQuery.Create(Self);
  ...
  mData.Post;
  try
    qry.SQL.Text:='Update T_WorkTaskWorkPrice set FWorkTaskPartsID=:FWorkTaskPartsID,FWorkBasePrice=:FWorkBasePrice,FWorkResizePrice=:FWorkResizePrice,'
...
procedure TFrm_WorkTaskWorkPrice.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TFrm_WorkTaskWorkPrice.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action:=cafree;
  Frm_WorkTaskWorkPrice:=nil;
end;

procedure TFrm_WorkTaskWorkPrice.TB_EXITClick(Sender: TObject);
begin
  inherited;
  self.Close;
end;

procedure TFrm_WorkTaskWorkPrice.MyTreeViewChange(Sender: TObject; Node: TTreeNode);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
  ...
  begin
    ...
    Act_TS_WorkTaskWorkPrice.Execute;//页选项设置
...
procedure TFrm_WorkTaskWorkPrice.MyTreeViewExpanding(Sender: TObject;
...
begin
  ...
  qry.SQL.Text:='Select FNumber, FParentNumber,FName,'
               +'Isleaf from VT_WorkTaskWorkPrice with(nolock) where FParentNumber= '''
...
procedure TFrm_WorkTaskWorkPrice.TB_RefClick(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
  ...
  begin
    If not TreeVeiwListWhere(qry,MyTreeView,'VT_WorkTaskWorkPrice',' and FDepartmentID='+IntToStr(UserFDepartmentID) ) then
       Application.MessageBox(PChar('树型结构显示错误，请联系系统开发人员'), '错误', MB_OK +MB_ICONSTOP);
  end;
  If UserFDepartmentsort=2 then //子公司
  begin
    If not TreeVeiwList(qry,MyTreeView,'VT_WorkTaskWorkPrice' ) then
       Application.MessageBox(PChar('树型结构显示错误，请联系系统开发人员'), '错误', MB_OK +MB_ICONSTOP);
  end;

end;

procedure TFrm_WorkTaskWorkPrice.OKBtnClick(Sender: TObject);
var qry: TADOQuery;
...
begin
  ...
  begin
    try
      qry.SQL.Text:='Select * from V_WorkTaskWorkPrice with(nolock) where FDepartmentID=:FDepartmentID and FWorkTaskPartsID=:FWorkTaskPartsID and FWorkCarryJobID=:FWorkCarryJobID';
...
procedure TFrm_WorkTaskWorkPrice.TB_AppClick(Sender: TObject);
...
procedure TFrm_WorkTaskWorkPrice.TB_EditClick(Sender: TObject);
var
 qry:TADOquery;
begin
  qry:=TAdoQuery.Create(self);
  ...
  try
    qry.SQL.Text:='Select * from V_WorkTaskPrice with(nolock) where FWorkTaskPartsID=:FWorkTaskPartsID and FWorkCarryJobID=:FWorkCarryJobID';
    qry.Parameters.FindParam('FWorkTaskPartsID').Value:=mdata['FWorkTaskPartsID'];
...
//    qry.Parameters.FindParam('FWorkCarryJob').Value:=Trim(E_FWorkCarryJob.Text);
  ...
  if qry.RecordCount>0 then
  begin
    application.MessageBox(Pchar('部件【'+Trim(Mdata['FWorkTaskPartsName'])+'】【'+Trim(Mdata['FWorkTaskPartsModel'])+'】【'+Trim(Mdata['FWorkCarryJob'])+'】已启用不允许修改！'),'系统提示',MB_ICONINFORMATION);
    Exit;
  end;
...
procedure TFrm_WorkTaskWorkPrice.TB_DelClick(Sender: TObject);
var
  qry:TADOQuery;
...
begin
  ...
  try
    qry.SQL.Text:='Select * from T_WorkTaskPrice where FWorkPriceID=:FWorkPriceID';
  ...
  if qry.RecordCount>0 then
  begin
    application.MessageBox(Pchar('部件【'+Trim(Mdata['FWorkTaskPartsName'])+'】【'+Trim(Mdata['FWorkTaskPartsModel'])+'】【'+Trim(Mdata['FWorkCarryJob'])+'】已启用不允许删除！'),'系统提示',MB_ICONINFORMATION);
    Exit;
  end;

  If application.MessageBox(Pchar('警告：是否删除：【'+Trim(Mdata['FWorkTaskPartsName'])+'】【'+Trim(Mdata['FWorkTaskPartsModel'])+'】【'+Trim(Mdata['FWorkCarryJob'])+'】的信息？'+chr(13)+chr(13)
...
procedure TFrm_WorkTaskWorkPrice.CancelBtnClick(Sender: TObject);
begin
  inherited;
  L_title.Caption:=UserFDepartmentName+'外协工管单价';
  mdata.Close;
...
procedure TFrm_WorkTaskWorkPrice.FormCreate(Sender: TObject);
begin
  inherited;
  TB_App.Enabled:= mdlData.CheckFrmRights(TFrm_WorkTaskWorkPrice.ClassName,'新增');
  TB_Price.Enabled:= mdlData.CheckFrmRights(TFrm_WorkTaskWorkPrice.ClassName,'新增');
  TB_Prin.Enabled:= mdlData.CheckFrmRights(TFrm_WorkTaskWorkPrice.ClassName,'打印');
  N_Out.Enabled:= mdlData.CheckFrmRights(TFrm_WorkTaskWorkPrice.ClassName,'打印');

end;

procedure TFrm_WorkTaskWorkPrice.FormShow(Sender: TObject);
begin
  TB_Ref.Click;
end;

procedure TFrm_WorkTaskWorkPrice.SelPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var selValue : string;
    qry,qry1: TADOQuery;
begin
  ...
  try
    qry.SQL.Text:='Select * from VT_WorkTaskParts with(nolock) where FDepartmentID=:FDepartmentID order by FNumber';
    qry.Parameters.FindParam('FDepartmentID').Value:=UserFDepartmentID;
    qry.Open;
  finally
     ;
  ...
  begin
    try
      qry1.SQL.Text:='Select * from V_WorkTaskParts with(nolock) where FWorkTaskPartsID=:FWorkTaskPartsID order by FNumber';
...
procedure TFrm_WorkTaskWorkPrice.E_FAgentDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var selValue : string;
    qry,qry1: TADOQuery;
begin
  ...
  //
  try
    qry.SQL.Text:='Select * from V_WorkTaskWorkPrice with(nolock) where FWorkCarryJobID=:FWorkCarryJobID and FProductID=:FProductID and FDepartmentID=:FDepartmentID';
    qry.Parameters.FindParam('FWorkCarryJobID').Value:=StrToInt(Trim(E_FWorkCarryJobID.Text));
  ...
  begin
    try
      qry.SQL.Text:='Select * from V_WorkTaskWorkPrice with(nolock) where FWorkPriceID=0';
      qry.Open;
    finally
       ;
    end;
    ...
    mdata.Active:=True;
    mdata.Edit;
    mdata['FWorkCarryJob']:=Trim(E_FWorkCarryJob.Text);
    mdata['FWorkCarryJobID']:=E_FWorkCarryJobID.EditValue;
    mdata.Post;
    Sel.Visible:=True;
  ...
  begin
    //取最大的工管执行价
    try
      qry.SQL.Text:='select * from V_WorkTaskWorkPrice with(nolock) where FDepartmentID=:FDepartmentID1 and FProductID=:FProductID '
                   +'and FWorkCarryJob=(Select max(FWorkCarryJob) from V_WorkTaskWorkPrice where FDepartmentID=:FDepartmentID2)';
      qry.Parameters.FindParam('FDepartmentID1').Value:=UserFDepartmentID;
      qry.Parameters.FindParam('FProductID').Value:=StrToInt(Trim(E_FProductID.Text));
      qry.Parameters.FindParam('FDepartmentID2').Value:=UserFDepartmentID;
      qry.Open;
    ...
    begin
      try
        qry1.SQL.Text:='Select *,0.00000 as FWorkBasePrice,0.00000 as FWorkResizePrice,0.00000 as FWorkPrice from V_WorkTaskParts with(nolock) '
...
procedure TFrm_WorkTaskWorkPrice.E_FWorkCarryJobDblClick(Sender: TObject);
begin
  inherited;
  SB_App.Enabled:=True;
...
procedure TFrm_WorkTaskWorkPrice.SB_AppClick(Sender: TObject);
var qry: TADOQuery;
    a:Integer;
begin
...
procedure TFrm_WorkTaskWorkPrice.MDataBeforePost(DataSet: TDataSet);
begin
  inherited;
  DataSet['FWorkPrice']:=DataSet['FWorkBasePrice']+DataSet['FWorkResizePrice'];
end;

procedure TFrm_WorkTaskWorkPrice.E_FProductNamePropertiesButtonClick(
...
procedure TFrm_WorkTaskWorkPrice.TB_PrinClick(Sender: TObject);
begin
  inherited;
...
procedure TFrm_WorkTaskWorkPrice.TB_PriceClick(Sender: TObject);
begin
  inherited;
  SB_App.Enabled:=True;
  E_FWorkCarryJobYear.Enabled:=True;
...
procedure TFrm_WorkTaskWorkPrice.N_EditClick(Sender: TObject);
begin
  inherited;
...
procedure TFrm_WorkTaskWorkPrice.N_FilterClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_WorkTaskWorkPrice.N_SortClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_WorkTaskWorkPrice.N_GroupClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_WorkTaskWorkPrice.N_SetClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_WorkTaskWorkPrice.N_OutClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_WorkTaskWorkPrice.cxGridContextPopup(Sender: TObject;
...
procedure TFrm_WorkTaskWorkPrice.Act_MenuExecute(Sender: TObject);
Var i:Integer;
begin
  //1、设置
...
procedure TFrm_WorkTaskWorkPrice.Act_Menu_SetExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_WorkTaskWorkPrice.Act_Set_OpenExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_WorkTaskWorkPrice.Act_Set_CloseExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_WorkTaskWorkPrice.Act_TS_WorkTaskWorkPriceExecute(
  Sender: TObject);
var
  qry:TADOquery;
begin
  ...
  qry.ConnectionString:=mdlData.ReadConnStr;
  qry.EnableBCD:=False;//小数位数>4
  If not MyItemList(qry,Mdata,'V_WorkTaskWorkPrice','FFullNumber',ListItemFNumber,'FFullNumber') then
  begin
    mdata.Open;
...
procedure TFrm_WorkTaskWorkPrice.Act_StatusExecute(Sender: TObject);
begin
  inherited;
  If P_Isleaf=1 then
  begin
    TB_Edit.Visible:= mdlData.CheckFrmRights(TFrm_WorkTaskWorkPrice.ClassName,'修改');
    TB_Del.Visible:=mdlData.CheckFrmRights(TFrm_WorkTaskWorkPrice.ClassName,'删除');
  end
  else
  begin
...
procedure TFrm_WorkTaskWorkPrice.E_FProductIDPropertiesEditValueChanged(
  Sender: TObject);
begin
  inherited;
