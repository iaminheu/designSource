...
uses
  ...
  cxGroupBox, cxCheckBox, RzShellDialogs,PROGRASS,cxCurrencyEdit, RzTabs, cxLookupEdit, cxDBLookupEdit,IniFiles,
  jpeg;

type
  TFrm_DesignBOMPickup = class(TFrm_GridBass)
    MData: TdxMemData;
...
var
  Frm_DesignBOMPickup: TFrm_DesignBOMPickup;
...
{$R *.dfm}
function TFrm_DesignBOMPickup.GetDsnFile: string;
const
 schema = 'schema.ini';
begin
...
function TFrm_DesignBOMPickup.GetTempFile: string;
const
 cTempFile='imp.txt';
begin
...
procedure TFrm_DesignBOMPickup.WriteODBC(IniFile: TIniFile);
const
  ODBC = 'ODBC';
begin
...
procedure TFrm_DesignBOMPickup.WriteDbFile(IniFile: TIniFile; DbFile: string);
begin
...
function TFrm_DesignBOMPickup.ReadSourceConnStr: string;
const
...
procedure TFrm_DesignBOMPickup.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TFrm_DesignBOMPickup.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action:=cafree;
  Frm_DesignBOMPickup:=nil;
end;

procedure TFrm_DesignBOMPickup.TB_EXITClick(Sender: TObject);
begin
  inherited;
  self.Close;
end;

procedure TFrm_DesignBOMPickup.FormCreate(Sender: TObject);
begin
  inherited;
  P_i:=True;
...
procedure TFrm_DesignBOMPickup.TB_InputClick(Sender: TObject);
var
  proc:TADOStoredProc;
  qry,qry1: TADOQuery;
begin
...
procedure TFrm_DesignBOMPickup.MDataAfterEdit(DataSet: TDataSet);
begin
  inherited;
  DataSet['IsEdit']:=True;
...
procedure TFrm_DesignBOMPickup.TB_shClick(Sender: TObject);
var
  qry:TADOQuery;
  proc:TADOStoredProc;
  P_gg:string;
...
procedure TFrm_DesignBOMPickup.FormShow(Sender: TObject);
var
  qry:TADOQuery;
begin
  qry:=TADOQuery.Create(Self);
...
procedure TFrm_DesignBOMPickup.ToolButton3Click(Sender: TObject);
begin
  inherited;
  ExportToExcel(cxGrid,True,TRue);
end;

procedure TFrm_DesignBOMPickup.TB_clClick(Sender: TObject);
var
  qry:TADOQuery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_DesignBOMPickup.TB_InClick(Sender: TObject);
Var qry:TADOQuery;
   // P_mc,P_gg:string;
//    IsEdit:bool;
begin
  ...
  begin
    ...
    Frm_progbar.Show;
    qry.SQL.Text:='Insert into T_Temp_DesignText(xh,A4,th,mc,gg,cl,ssth,sl,zsl,djjz,djde,Remark,bjjb,UserNum) '
                 +'Values(:xh,:A4,:th,:mc,:gg,:cl,:ssth,:sl,:zsl,:djjz,:djde,:Remark,:bjjb,:UserNum)';
    qry.ExecSQL;
    with mdata do
    begin
      DisableControls;
    ...
    Messagedlg('本体院设计文本已成功提取！',mtInformation,[mbok],0);
  end;
...
procedure TFrm_DesignBOMPickup.E_th1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var selValue : string;
    qry: TADOQuery;
...
//  T_DesignBOM_Structure
  ...
  begin
    try
      qry.SQL.Text:='Insert into T_DesignBOM_Structure(Code,Parent,PartID,sl,FFullNumber,zsl,FFullName) '
...
procedure TFrm_DesignBOMPickup.TB_DownClick(Sender: TObject);
begin
  inherited;
  If P_i then
  begin
...
procedure TFrm_DesignBOMPickup.CB_2PropertiesChange(Sender: TObject);
Var i:Integer;
begin
...
procedure TFrm_DesignBOMPickup.E_th11DblClick(Sender: TObject);
begin
  inherited;
  TB_sh.Visible:=True;
  TB_cl.Visible:=True;
...
procedure TFrm_DesignBOMPickup.ToolButton2Click(Sender: TObject);
var
  qry:TADOQuery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_DesignBOMPickup.ToolButton4Click(Sender: TObject);
var
  qry:TADOQuery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_DesignBOMPickup.ToolButton6Click(Sender: TObject);
var
  qry:TADOQuery;
begin
  qry:=TADOQuery.Create(self);
...
procedure TFrm_DesignBOMPickup.Button1Click(Sender: TObject);
Var qry:TADOQuery;
begin
  qry:=TADOQuery.Create(self);
  qry.Connection:=mdlData.conn;
