...
type
  TFrm_lyl = class(TFrm_GridBass)
    ...
    Label1: TLabel;
    E_FLylScheme: TcxButtonEdit;
    FReMark: TcxGridDBColumn;
    FModel: TcxGridDBColumn;
    FSizeStar: TcxGridDBColumn;
    FSizeEnd: TcxGridDBColumn;
    th: TcxGridDBColumn;
    FSort: TcxGridDBColumn;
...
begin
 // result:=False;
  ...
  try
    qry.SQL.Text:='Delete from T_LYL where FProductID=:FProductID and FLylScheme=:FLylScheme ';
    with qry.Parameters do
    begin
      FindParam('FProductID').value:=Trim(E_FProductID.Text);
      FindParam('FLylScheme').value:=Trim(E_FLylScheme.Text);
    end;
...
begin
  ...
  try
    qry.SQL.Text:='Insert into T_LYL(FLYLType,FNum,lyl,th,mc,FMakeParts,FName,FModel,FSizeStar,FSizeEnd,FRemark,FSort,FLylScheme,FDepartmentID,FProductID) '
                 +'values(:FLYLType,:FNum,:lyl,:th,:mc,:FMakeParts,:FName,:FModel,:FSizeStar,:FSizeEnd,:FRemark,:FSort,:FLylScheme,:FDepartmentID,:FProductID)';
     mData.First;
     ...
     begin
       ...
       begin
         ...
         with qry.Parameters do
         begin
           FindParam('FLYLType').value:=mdata['FLYLType'];
           FindParam('FNum').value:=mdata['FNum'];
           FindParam('lyl').value:=mdata['lyl'];
           ...
           FindParam('FSizeStar').value:=mdata['FSizeStar'];
           FindParam('FSizeEnd').value:=mdata['FSizeEnd'];
           FindParam('FRemark').value:=mdata['FRemark'];
           FindParam('FSort').value:=mdata['FSort'];
           FindParam('FLylScheme').value:=Trim(E_FLylScheme.Text);
...
procedure TFrm_lyl.FormResize(Sender: TObject);
begin
  inherited;
  L_title.Caption:='利用率库操作';
  Self.Caption:='利用率库操作';
end;

procedure TFrm_lyl.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action:=cafree;
  Frm_lyl:=nil;
...
procedure TFrm_lyl.TB_EXITClick(Sender: TObject);
begin
  inherited;
  self.Close;
end;

procedure TFrm_lyl.MyTreeViewChange(Sender: TObject; Node: TTreeNode);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
  ...
  begin
    ...
    begin
      If not MyItemList(qry,Mdata,'V_LYL','FFullNumber',ListItemFNumber,'FFullNumber') then
      begin
        mdata.Open;
      ...
      begin
        ...
        E_FLylScheme.Text:= mdata['FLylScheme'];
...
procedure TFrm_lyl.MyTreeViewExpanding(Sender: TObject;
...
begin
  ...
  qry.SQL.Text:='Select * from VT_LYL where FParentNumber= '''
...
procedure TFrm_lyl.OKBtnClick(Sender: TObject);
var qry: TADOQuery;
...
begin
  ...
  begin
    try
      qry.SQL.Text:='Select * from V_LYL where FLylScheme=:FLylScheme and FDepartmentID=:FDepartmentID';
      qry.Parameters.FindParam('FLylScheme').Value:=mdata['FLylScheme'];
  ...
  E_FLylScheme.Enabled:=False;

end;



procedure TFrm_lyl.TB_appClick(Sender: TObject);
var   qry: TADOQuery;
      i:Integer;
begin
  qry:=TADOQuery.Create(self);
  ...
  try
    qry.SQL.Text:='select * from V_lyl with(nolock) where FLYLID=0';
    qry.Open;
  finally
    ;
  end;
...
procedure TFrm_lyl.TB_EDITClick(Sender: TObject);
var
...
procedure TFrm_lyl.TB_DELClick(Sender: TObject);
var
  qry:TADOQuery;
...
begin
  ...
  If application.MessageBox(Pchar('警告：是否删除部件类别为：【'+Trim(Mdata['FLylScheme'])+'】的单据？'+chr(13)+chr(13)
...
procedure TFrm_lyl.CancelBtnClick(Sender: TObject);
begin
  inherited;
...
procedure TFrm_lyl.E_FProductNamePropertiesButtonClick(
...
procedure TFrm_lyl.FormCreate(Sender: TObject);
begin
  inherited;
  TB_App.Enabled:= mdlData.CheckFrmRights(TFrm_lyl.ClassName,'新增');
  TB_Edit.Enabled:= mdlData.CheckFrmRights(TFrm_lyl.ClassName,'修改');
  TB_Del.Enabled:= mdlData.CheckFrmRights(TFrm_lyl.ClassName,'删除');
  TB_Prin.Enabled:= mdlData.CheckFrmRights(TFrm_lyl.ClassName,'打印');

end;

procedure TFrm_lyl.Act_RefExecute(Sender: TObject);
var
  qry:TADOquery;
begin
  qry:=TADOQuery.Create(self);
  qry.Connection:=mdlData.conn;
  If not TreeVeiwListUserID(qry,MyTreeView,'VT_LYL',UserFDepartmentID) then
  begin
...
procedure TFrm_lyl.FormShow(Sender: TObject);
begin
  inherited;
  Act_Ref.Execute;
...
procedure TFrm_lyl.Act_MenuExecute(Sender: TObject);
Var i:Integer;
begin
  //1、设置
...
procedure TFrm_lyl.Act_Menu_SetExecute(Sender: TObject);
begin
  inherited;
...
procedure TFrm_lyl.cxGridContextPopup(Sender: TObject; MousePos: TPoint;
  var Handled: Boolean);
begin
  inherited;
  GV:='cxGV';
...
procedure TFrm_lyl.N_EditClick(Sender: TObject);
begin
  inherited;
...
procedure TFrm_lyl.N_FilterClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_lyl.N_SortClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_lyl.N_GroupClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_lyl.N_SetClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
...
procedure TFrm_lyl.N_OutClick(Sender: TObject);
begin
  inherited;
   Mn:=TMenuItem(Sender).Name;
   Act_Menu.Execute;
